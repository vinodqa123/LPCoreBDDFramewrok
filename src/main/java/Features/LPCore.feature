Feature: LP Core functional Regression test
 #Manual Entry Payment feature
  ...
  Scenario Outline: Verify Contacless Payment with different type of cards(VISA/MasterCard/Amex/Discover..)
    Given User is already on Home Page
    When  Select payment type as Contactless
    When Enter the Patient details ReferenceNumber:"<ReferenceNumber>",FirstName:"<firstname>",LastName:"<lastname>",Phone/Email:"<phone>",Amount:"<Amount>"
    And Click on Submit button
            
    Examples:
    | ReferenceNumber   | firstname | lastname |phone |Amount |cardNumber |address|state|city|cvv|
    | 23290-23           | John      | smith    |9548406651 |3 |4012002000060016 |Madhapur|Hyderabad|Telangana|123|
